...
if(isset($_POST['save_suapm'])){
    ...
    $sl_id_sach = "SELECT * FROM sach WHERE id_sach='$mas' ";
    $res_sls = mysqli_query($conn, $sl_id_sach);

	$sl_id_cb = "SELECT * FROM thuthu WHERE id='$cb' ";
			        ...
			        while ($rows=$res_sls->fetch_assoc()) {
			        	while ($rowcb=$res_cb->fetch_assoc()) {
						    ...
						    if (mysqli_query($conn, $sql_ud)) {
						        ...
						        echo "<script type='text/javascript'> document.location ='TrangChuThuThu.php?id_dn=$tendn&xpm'; </script>";
						    } else {
