...
if (isset($_POST['xacnhan'])) {
	...
	$sql_check = "SELECT * from danhsachmuon WHERE tinhtrang='3' and nm='$tendn' ";
	//tinh trang 0 đã trả, 1 chưa trả, 3 chờ duyệt
	...
	if (mysqli_num_rows($res_check) > 0) {
		...
		while ($row = $res_sl_dsm-> fetch_assoc()) {
			...
			$slm = $row['soluongmuon'];
			//ngay tra
			$ngaymuon = date("Y")."-".date("m")."-".date("d");
			$nt = ngaytra($songaytra, date("Y"), date("m"), date("d"));
			$sql_ud_dsm = "UPDATE `danhsachmuon` SET `songaytra`='$songaytra', `ngaymuon`='$ngaymuon', `ngaytra`='$nt', `tinhtrang`='3', `lanmuon`= `lanmuon`+1 WHERE tinhtrang='' and nm='$tendn' ";
			$conn->query($sql_ud_dsm);
			$sql = "INSERT INTO `phieumuon`(`nguoimuon`, `tensach`, `soluongmuon`, `ngaymuon`, `ngaytra`, `tinhtrang`, `canbo`, `noidung`) VALUES ('$nm','$ts','$slm', '$ngaymuon','$nt','5', '999', '0')";
			$conn->query($sql);
		}
